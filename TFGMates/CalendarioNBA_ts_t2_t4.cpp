...
int main() {
    ...
    ofstream archivo2("resultados.txt");  // Crea o abre el archivo
    ...
    vector<int> T_values = { 100, 200, 400 };
    vector<int> M_values;
    for (int i = 1; i < 51; i++) {
        M_values.push_back(i);
    }
    M_values.push_back(82);
    M_values.push_back(410);
    M_values.push_back(820);
    vector<double> alpha_values = { 0.95, 0.97, 0.99 };

    vector<double> mejores_distancias;
    ...
    for (int T_inicial : T_values) {
        
        for (int M : M_values) {
            for (double alpha : alpha_values) {
                double t_inicial = T_inicial;
                    ...
                    while (j1 < TOTAL_JORNADAS) {
                        ...
                        while (j2 < TOTAL_JORNADAS) {
                            ...
                            if (j2 - j1 == 1) {
                                ...
                                distancia_inicial = calculaDistancias2(j1, j1 - 1, j1 + 1, viajes) + calculaDistancias2(j2, j2 - 1, j2 + 1, viajes);
                                distancia_final = calculaDistancias2(j2, j1 - 1, j1 + 1, viajes) + calculaDistancias2(j1, j2 - 1, j2 + 1, viajes);
                            }
                ...
                cout << "Mejor distancia encontrada para T = " << T_inicial << ", M = " << M << " y alpha = " << alpha << " : " << distancia_mejor << endl;
                archivo2 << "Mejor distancia encontrada para T = " << T_inicial << ", M = " << M << " y alpha = " << alpha << " : " << distancia_mejor << endl;
                viajes = copiar_calendario(modelo);
                mejor_calendario = copiar_calendario(modelo);
                mejores_distancias.push_back(distancia_mejor);
                distancia = distancia_modelo;
                distancia_mejor = distancia_modelo;
            }
        }
    }
    ...
    double min = distancia_modelo;
    for (int i = 0; i < mejores_distancias.size(); i++) {
        if (min > mejores_distancias[i]) {
            min = mejores_distancias[i];
        }
    }

    cout << "Mejor solución: " << min << endl;
    archivo2 << "Mejor solución: " << min << endl;
