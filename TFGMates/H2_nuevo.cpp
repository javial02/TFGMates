        ...
        if (viajes[i][k] != -1) {
            ...
            if (viajes[i][k2] == -1) {
                ...
                if (kantes >= 0 && kdespues <= TOTAL_JORNADAS - 1) {
                    total_dist += distanciasNBA[viajes[i][kantes]][viajes[i][kdespues]];
                }
                else if (k2antes < 0) {
                    total_dist += distanciasNBA[viajes[i][kdespues]][i];
                }
                else if (k2despues > TOTAL_JORNADAS - 1) {
                    total_dist += distanciasNBA[viajes[i][kantes]][i];
                }
            }
...
int main() {
    ...
    while (k1 < TOTAL_JORNADAS) {
        ...
        while (k2 < TOTAL_JORNADAS) {
            ...
            if (distancia_final < distancia_inicial) {
                ...
                distanciaTotal -= diferencia;
                cambiaJornadas_h2(k1, k2, viajes);
                cout << "Cambio de dias: " << k1 << " " << k2 << "disminuyendo" << diferencia << "millas" << endl;
                k1 = -1;
                break;
            }
            k2++;
    ...
    cout << "Distancia final: " << distanciaTotal << endl;
