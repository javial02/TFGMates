...
double calculaDistancias_h2(int k, int k2, const vector<vector<int>>& viajes) {
    double total_dist = 0;
    for (int i = 0; i < N; i++) {
        if (viajes[i][k] != -1) {
            int kantes = partido_anterior(viajes, i, k);
            int kdespues = partido_posterior(viajes, i, k);
            int k2antes = partido_anterior(viajes, i, k2);
            int k2despues = partido_posterior(viajes, i, k2);
            if (k2antes >= 0 && k2despues <= TOTAL_JORNADAS - 1) {
                total_dist += distanciasNBA[viajes[i][k2antes]][viajes[i][k]] + distanciasNBA[viajes[i][k]][viajes[i][k2despues]];
            }
            else if (k2antes < 0) {
                total_dist += distanciasNBA[viajes[i][k]][viajes[i][k2despues]] + distanciasNBA[viajes[i][k]][i];
            }
            else if (k2despues > TOTAL_JORNADAS - 1) {
                total_dist += distanciasNBA[viajes[i][k2antes]][viajes[i][k]] + distanciasNBA[viajes[i][k]][i];
            }


            if (viajes[i][k2] != -1) {
                if (kantes >= 0 && kdespues <= TOTAL_JORNADAS - 1) {
                    total_dist += distanciasNBA[viajes[i][kantes]][viajes[i][k2]] + distanciasNBA[viajes[i][k2]][viajes[i][kdespues]];
                }
                else if (kantes < 0) {
                    total_dist += distanciasNBA[viajes[i][k2]][viajes[i][kdespues]] + distanciasNBA[viajes[i][k2]][i];
                }
                else if (kdespues > TOTAL_JORNADAS - 1) {
                    total_dist += distanciasNBA[viajes[i][kantes]][viajes[i][k2]] + distanciasNBA[viajes[i][k2]][i];
                }
            }
...
void temple_simulado(vector<vector<int>>& viajes, double& distancia, double t_inicial, double t_minima, int M, double alpha) {
    ...
    while (t_inicial > t_minima) {
        ...
        for (int i = 0; i < M; i++) {
            ...
            distancia_inicial = calculaDistancias_h2(k1, k1, viajes) + calculaDistancias_h2(k2, k2, viajes);

            if (k1 < k2) {
                if (k2 - k1 == 1) {

                    distancia_final = calculaDistancias_h2(k2, k1,viajes) + calculaDistancias_h2(k1, k2, viajes);
                }
                else {
        ...
        while (j1 < TOTAL_JORNADAS) {
            ...
            while (j2 < TOTAL_JORNADAS) {
                distancia_inicial = calculaDistancias_h2(j1, j1, viajes) + calculaDistancias_h2(j2, j2, viajes);
                if (j2 - j1 == 1) {
                    distancia_final = calculaDistancias_h2(j2, j1,viajes) + calculaDistancias_h2(j1, j2,viajes);
                }
                else {
                    distancia_final = calculaDistancias_h2(j2, j1, viajes) + calculaDistancias_h2(j1, j2, viajes);
                }

                diferencia = distancia_final - distancia_inicial;
...
int main() {
    ...
    vector<vector<int>> mejor_calendario;
    double dist_mejor = distanciaTotal;
    ...
    for (double t_inicial : t) {
        for (int M : Ms) {
            for (double alpha : alphas) {
                temple_simulado(viajes, distanciaTotal, t_inicial, t_minimo, M, alpha);
